...
func getHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path != "/" {
		...
		asciiStr := figure.NewFigure(str, strings.ToLower(r.Header.Get("font")), false) // `false` means we'll replace non-ASCII chars with `?` // lowercasing font header values
		io.WriteString(w, asciiStr.String())
		return
	}
	fmt.Printf("got / request\n")
	str := "base path"
	asciiStr := figure.NewFigure(str, strings.ToLower(r.Header.Get("font")), true) // lowercasing font header values
	io.WriteString(w, asciiStr.String())
}
